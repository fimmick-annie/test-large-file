...
@section('page_title', 'Campaign Offer #'.$offer->id.' Coupon Pool Confirmation')
...
@endforeach
				...
				<p>
					<div class=' col-sm-12'>
						<button type="button" class="btn btn-danger" id="importButton">Confirm Import</button>
					</div>
				</p>
			</div>
		</div>
	...
	$(document).ready(function()  {
		...
		$("#importButton").on('click', function()  {
			...
			$.ajax({
				...
				dataType: "json",
				url: '{{ route("foso.campaigns.offer.couponpool.confirm.json", ["offer_code"=>$offerCode]) }}?uniqid={{ $uniqid }}',
				success: function (result)  {

					switch (result.status)  {
						case 0:  {
							location.href = '{{ route("foso.campaigns.offer.couponpool.html", ["offer_code"=>$offerCode]) }}';
							return;
						}
