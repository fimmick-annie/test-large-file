...
@section('page_title', 'Campaign Offer #'.$offer->id.' Coupons')
...
<ul class="nav nav-tabs" role="tablist">
	...
	<li class="nav-item"> <a class="nav-link active" href='{{ route("foso.campaigns.offer.coupons.html", ["offer_code"=>$offerCode]) }}' role="tab"><span class="hidden-sm-up"><i class="fa fa-tags"></i></span> <span class="hidden-xs-down">Coupons</span></a> </li>
...
<div class="card">
	<div class='card-body'>
		<div class="row">
			<div class="col-sm-12">
				Default showing record from 30 days before.
			</div>
		</div>
	</div>
	...
	$(document).ready(function()  {
		...
		$("#clearButton").click(function()  {
			...
			$.ajax({
				...
				data: formData,
				dataType: "json",
				url: '{{ route("foso.campaigns.offer.coupons.clearwhitelisted.json", ["offer_code"=>$offerCode]) }}',
		...
		_table = $("#dataTable").DataTable({
			...
			buttons: ["csv", "excel", "copy", {
				...
				action: function (e, dt, node, config) {
					...
					var url = "?from="+fromDate+"&to="+toDate;
					url = '{{ route('foso.campaigns.offer.coupons.csv.file', ['offer_code' => $offerCode]) }}' + url;
			...
			ajax: {
				url: '{{ route("foso.campaigns.offer.coupons.json", ["offer_code"=>$offerCode]) }}',
				data:  function(data)  {
