...
@endif
		...
		<div class="wrapper">
			...
			<form id="form" class="form__redemption"><center>
				...
				<input type="hidden" name="pickedRedemptionStoreCode" id="pickedRedemptionStoreCode" value="{{ $selectedRedemptionStore }}" />

				<div class="coupon__slide" data-holdTime='1000'>
		...
		<script type="text/javascript">
			...
			var slideToEdgeTime;
			var mouseUpTime;
			var holdTime = parseInt( document.querySelector('[data-holdTime]').dataset.holdtime);
			...
			function mouseupHandler(ev) {
				if (!isSuccess) {
					...
					if(slideToEdgeTime ) {
						...
						// reach edge and hold > 1 second
						console.log('Slided To Edge And holded', mouseUpTime - slideToEdgeTime, 'ms')
						if(mouseUpTime - slideToEdgeTime >= holdTime) {
							success();
						} else {
							confirmProgress.animate({
								width: 32
			...
			function mousemoveHandler(ev) {
				...
				if (diffX === slideBox.width() - slideBtn.width()) {
					slideToEdgeTime = new Date().getTime();
					// console.log('Reach edge', slideToEdgeTime)
					isSuccess = true;
				} else {
					isSuccess = false;
				}
