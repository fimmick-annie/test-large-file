...
FIRST_SENTS = json.load(open('/content/drive/MyDrive/Llama2_ch/data/first_sentences.json'))
Q_CLS = json.load(open('/content/drive/MyDrive/Llama2_ch/data/question_cls.json'))
DATAS = json.load(open('/content/drive/MyDrive/Llama2_ch/data/database.json'))# change it by first_sent cls

RECOMM_ARTICLE_CACHE = None
SEARCH_KEY_CACHE = None
QS = None
QN = None
...
def setup():
    global tokenizer, model, device, share, port, max_memory, s_model, index, DATAS, RECOMM_ARTICLE_CACHE, FIRST_SENTS, Q_CLS, QS, QN, SEARCH_KEY_CACHE
    if args.use_vllm:
    ...
    s_model = SentenceTransformer(args.similarity_model)
    s_model.eval()
    database_embeddings = s_model.encode(list(DATAS.keys()), normalize_embeddings=True)
    #Move param to arg?
    dim, measure = database_embeddings.shape[1], faiss.METRIC_L2
    param = 'Flat'
    index = faiss.index_factory(dim, param, measure)
    if not index.is_trained:
        index.train(database_embeddings)          
    index.add(database_embeddings)
    print('目前資料庫中有'+str(index.ntotal)+'筆資料')
...
def generate_prompt(instruction, response="", with_system_prompt=True, system_prompt=DEFAULT_SYSTEM_PROMPT):
    if with_system_prompt is True:
        prompt = TEMPLATE_WITH_SYSTEM_PROMPT.format_map({'instruction': '使用者:'+instruction+" 助手:",'system_prompt': system_prompt})
    else:
        prompt = TEMPLATE_WITHOUT_SYSTEM_PROMPT.format_map({'instruction': '使用者:'+instruction+" 助手:"})
    prompt += response
    return prompt
...
def get_recomm_from_database(ht, sm, qs, cache=None, add_condition=''):
    global SEARCH_KEY_CACHE
    topk = 10
    search_key_words = ''
    if cache == None:
        questions = []
        ...
        for sents in ht:
            if sents != ht[-1]:
                questions.append(sents[1])
            if sents != ht[0]:
                answers.append(sents[0])
        embeddings_k = sm.encode(qs, normalize_embeddings=True)
        embeddings_q = sm.encode(questions, normalize_embeddings=True)
        similarity = embeddings_k @ embeddings_q.T
            
        for ind,k in zip([np.argmax(i) for i in similarity], qs):
            search_key_dict[k+'++'] = answers[ind].strip()
        SEARCH_KEY_CACHE = copy.copy(search_key_dict)
    else: 
        if add_condition != '':
            SEARCH_KEY_CACHE[list(SEARCH_KEY_CACHE.keys())[-1]] = list(SEARCH_KEY_CACHE.values())[-1] +','+ add_condition.strip()
    search_key_words = ','.join([i[:-2]+':'+j for i,j in zip(SEARCH_KEY_CACHE.keys(), SEARCH_KEY_CACHE.values())])
    search_key_embedd = sm.encode(search_key_words, normalize_embeddings=True).reshape(1,-1)

    _, I = index.search(search_key_embedd, topk)
    return I[0]

def member_faq_search(ht, sm):
    topk = 3
    embeddings_k = sm.encode(ht[-1][0], normalize_embeddings=True).reshape(1,-1)
    distance, I = index.search(embeddings_k, topk)
...
def predict(
    ...
    global DATAS, RECOMM_ARTICLE_CACHE, FIRST_SENTS, Q_CLS, QS, QN, SEARCH_KEY_CACHE
    if len(system_prompt) == 0:
        system_prompt = DEFAULT_SYSTEM_PROMPT
        
    history[0][0] = history[0][0].strip()
    print("len(history):", len(history))
    print("history: ", history)
    history[-1][1] = ""
    if history[0][0] == '會員++':
        if len(history) == 1:
            history[-1][1] = FIRST_SENTS[history[0][0]]
            yield history
        elif history[-1][0] == '重問++':
            history[-1][1] = '請您再一次描述問題'
            yield history
        elif history[-2][0] == '重問++':
            answer_index, threshold = member_faq_search(history, s_model)
            if threshold[0] > 1:
                history[-1][1] = '您的問題可能需要與客服聯繫'
            else:
                answers = [str(i+1)+'.'+list(DATAS.values())[ind] for i,ind in enumerate(answer_index)]
                history[-1][1] = '\r\n'.join(['最符合的三個答案:']+answers+['沒解答到，與客服聯繫(聯繫++)/其他疑問(再問++)/有幫助到，結束:D(解答++)(這應該是個button)'])
            yield history
        elif history[-1][0] == '再問++':
            history[-1][1] = '還有什麼想要詢問的呢？請直接跟我說'
            yield history
        elif history[-2][0] == '再問++':
            answer_index, threshold = member_faq_search(history, s_model)
            if threshold[0] > 1:
                history[-1][1] = '沒有能找到較符合的回答，可能需要您重新描述問題'
            else:
                answers = [str(i+1)+'.'+list(DATAS.values())[ind] for i,ind in enumerate(answer_index)]
                history[-1][1] = '\r\n'.join(['最符合的三個答案:']+answers+['沒解答到，重新描述問題(重問++)/其他疑問(再問++)/有幫助到，結束:D(解答++)(這應該是個button)'])
            yield history
        else:
            answer_index, threshold = member_faq_search(history, s_model)
            if threshold[0] > 1:
        ...
        if history[0][0] in FIRST_SENTS.keys() and history[0][0] != '會員++':
            QS = Q_CLS[history[0][0]]
            QN = len(QS)+1
            history[0][0] = FIRST_SENTS[history[0][0]]
            SEARCH_KEY_CACHE = None
            
        if len(history) == QN or history[-1][0] == '推薦++':
            if SEARCH_KEY_CACHE == None:
                recomm_index = get_recomm_from_database(history, s_model, QS)
            else:
                recomm_index = get_recomm_from_database(history, s_model, QS, SEARCH_KEY_CACHE)
            recomm_data = list(DATAS.values())
            RECOMM_ARTICLE_CACHE = [recomm_data[i] for i in recomm_index]
            recomm_article = '\r\n'.join(RECOMM_ARTICLE_CACHE[:5])
            history[-1][1] = recomm_article + '\r\n需要重新推薦(重新++)/修改資訊(修改++)/增加資訊(增加++)/滿意結束:D(滿意++)(這應該是個button)'#應該是個button
            yield history
            ...
            history[-1][1] = '您想修改哪一個資訊: 美食地區/人數/用餐預算/用餐時段/用餐口味喜好(這應該是個button)'
            yield history
            
        elif history[-1][0] in [i+'++' for i in QS]:
            history[-1][1] = '您想修改'+history[-1][0][:-2]+'為..? 請直接輸入:D'
            yield history
            
        elif len(history) != 1 and history[-2][0] in [i+'++' for i in QS]:#['美食地區++', '人數++', '用餐預算++', '用餐時段++', '用餐口味喜好++']:
            SEARCH_KEY_CACHE[history[-2][0]] = history[-1][0]
            history[-1][1] = '已為您修改完成，要繼續修改還是重新推薦?修改++/推薦++(這應該是個button)'
            yield history
            
        elif history[-1][0] == '增加++':
            history[-1][1] = '請直接告訴我您想增加的想法！'
            yield history
            
        elif len(history) != 1 and history[-2][0] == '增加++':
            recomm_index = get_recomm_from_database(history, s_model, QS, SEARCH_KEY_CACHE, history[-1][0])
            recomm_data = list(DATAS.values())
            RECOMM_ARTICLE_CACHE = [recomm_data[i] for i in recomm_index]
            recomm_article = '\r\n'.join(RECOMM_ARTICLE_CACHE[:5])
            history[-1][1] = recomm_article + '\r\n需要重新推薦(重新++)/修改資訊(修改++)/增加資訊(增加++)/滿意結束:D(滿意++)(這應該是個button)'
            yield history
